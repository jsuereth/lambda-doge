...
sealed trait DogeSymbol {
  ...
  // TODO - Should we support pruning throughout the whole hierarchy?
  def isPruned: Boolean = true
...
abstract class AbstractDogeSymbol extends DogeSymbol {
  ...
  override def isPruned: Boolean = false
}

/** A symbol which has been pruned. */
case class PrunedSymbol(override val original: DogeSymbol, tpe: Type) extends AbstractDogeSymbol {
  /** All symbols must have a name for their current scope. */
  override def name: String = original.name
  override def isBuiltIn: Boolean = original.isBuiltIn
  override def isJava: Boolean = original.isJava
  override def isJavaClass: Boolean = original.isJavaClass
  override def isJavaMethod: Boolean = original.isJavaMethod
  override def isJavaConstructor: Boolean = original.isJavaConstructor
  override def isJavaField: Boolean = original.isJavaField
  override def isPruned: Boolean = true
  override def toString = s"${original} as $tpe"
}
